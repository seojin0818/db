...
/*
    ...
    제약조건
    ==> 데이터가 입력될 때 이상이 있는 데이터가 막는 기능의 오라클 개체
    ...
    종류 ]
        ...
        기본키(PRIMARY KEY) 제약조건 
            - 속성값으로 데이터를 구분할 수 있어야 함
              기본키 제약조건은 필수는 아니지만
              되도록이면 기본키 제약조건은 추가하는 것이 좋음
              
              기본키 = 유일키 + NOT NULL 제약조건
              
        유일키(UNIQUE KEY) 제약조건
            - 속성값이 다른 데이터와 구분이 되어야 함
              NULL 데이터는 입력할 수 있음
        ...
        참조키(외래키, FOREIGN KEY) 제약조건
            - 참조하는 테이블의 키값을 반드시 사용해야 하는 제약조건
              ...
              만약 테이블을 삭제하려고 하면
              먼저 참조하고 있는 테이블을 삭제하고 해당 테이블을 삭제해야 함
              
        NOT NULL 제약조건
            - 속성값으로 NULL 데이터가 입력되지 않도록 막는 제약조건
...
/*
        ...
        1. 문자형
            CHAR
                ...
                형식 ]
                    ...
                    CHAR(최대길이)
                    ==> 이 때 사용가능한 최대 길이는 2000 바이트까지만 사용 가능
            ...
            CLOB
                ...
                ==> 이 타입은 VARCHAR2가 4KB까지만 허락하기 때문에
                    그 이상의 글자수를 가진 데이터는 입력받지 못함
                    이 문제를 해결하기 위한 방식으로 만들어놓은 타입
                    
    참고 ]
    
        고정길이 문자형 ( CHAR가 처리속도 훨씬 빠름)
            ==> 문자의 길이를 미리 지정한 후
        ...
        2. 숫자형
            NUMBER
                ...
                참고 ]
                    ...
                    숫자1보다 큰 수는 입력받지 못함
                    숫자2보다 소수이하가 많으면 자동으로 반올림해서 입력함
    ...
    참고 ]
        ...
        데이터베이스에 따라 데이터 형태도 약간씩 달라짐
        요사이 ANSI 데이터 형태라고 해서
        모든 데이터베이스에 적용 가능한 공통 형태를 표준화협회에서 제시하고 있음
