...
/*
    ...
    오라클의 경우는
    계층형 결과를 볼 수 있는 문법을 만들어서 제공해주고 있음
...
SELECT
    ...
    -- START WITH 절이 있는 곳에서만 쓸 수 있는 의사 컬럼
FROM
    emp
START WITH
    mgr IS NULL
        ...
        ==> 이미 만들어진 테이블에 데이터를 추가하는 명령
            ...
            형식 ]
                ...
                VALUES(
                    테이블이 가지는 모든 필드의 데이터
                    만약 필드이름을 지정해 놓으면
                    위에서 지정한 순서대로 필드에 해당하는 데이터를 입력해야 함
                );
                
            주의 ]
            
                입력할 때는 필드의 순서에 맞게
                반드시 데이터의 개수와 순서를 일치시켜서 입력해야 함
                
            참고 ]
                
                만약 데이터가 준비되지 않아서 데이터가 부족할 경우에는
                    1. NULL로 그 부분을 대신 채우면 됨
                    
                    2. 필드이름을 지정할 때 생략하면 됨
...
/*
    ...
    **********
    참고 ]
        ...
        작업이 이뤄짐
        따라서 데이터베이스에 적용은 되어있지 않은 상태
        작업내용이 데이터베이스에 적용되기를 원한다면
        반드시 commit 명령을 실행해야 함
    ...
    ==> 현재 데이터 중에서 필요하지 않는 데이터를 삭제하는 명령
        ...
        ***
        참고 ]
            ...
            조건식을 제시하지 않으면 모든 데이터가 삭제됨
        
        참고 ]
            
            이 명령은 되도록이면 사용하지 않을 것을 권하는 명령
            
            현업에서는
            isShow CHAR(1)라는 필드를 준비해두고
            기본값은 'Y'
            그리고 삭제가 필요한 경우는 'N'으로 데이터를 수정함
            그리고 조회질의명령에서는
            반드시
                WHERE
                    isShow = 'Y'
...
/*
    ...
    참고 ]
        ...
        테이블 설계
            오라클은 정규화된 데이터를 기억하도록 하는 데이터베이스의 일종
            ( ==> 정형데이터베이스 )
            정규화 데이터란
            규칙이 정해진 데이터
        ...
        정규화 과정
            ...
            1. 제1정규화
                ==> 개체가 갖는 속성은 원자값을 가져야 함
                    
                ==> 결과물로 제1정규형
                    
            2. 제2정규화
                ==> 기본키에 대해서 모든 키는 완전함수 종속이어야 함
                ==> 결과물로 제2정규형
